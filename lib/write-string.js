...
function writeString(inputString, inputYaml, _options) {
  ...
  return new Promise((resolve, reject) => {
      ...
      if (!foundInitialDashes) {
        ...
        if (!foundClosingDashes) {
          if (input.trim() === FM_SEPARATOR) {
            ...
            //   } catch(err) {
            //     reject(`Error parsing YAML in front matter\n\n${err.message}`);
    ...
    rl.on('close', () => {
      ...
      const resultYamlFM = combineYaml(yamlFMContents.join('\n'), inputYaml);
      // fmYaml._contents = otherContents.join('\n');
      resolve([FM_SEPARATOR, resultYamlFM, FM_SEPARATOR, otherContents].join('\n'));
    });
